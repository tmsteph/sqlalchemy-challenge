...
Base.prepare(engine, reflex=True)
...
def precipitation():
    ...
    return jsonify(precipitation_date_tabs)
...
def stations():
    ...
    #create session (link) from Pythot to the DB
    session = Session(engine)
    results = session.query(Station.name).all()
    station_deatils = list(np.ravel(results))
...
def tobs():
    ...
    #Query Measurements for the latest date and calculate query_start_date
    ...
    q_station_list = (session.query(Measurement.station, func.count(Measurement.station))
                             .groupby(Measurement.station, func.count(Measurement.station))
                             .order_by(func.count(Measurement.station).desc())
                             .all())
    station_hno = q_station_list[0][0]
    print(station_hno

    # Return a list of tabs for the year before the final date
...
# Check for valid endtry of start date
    ...
    if valid_entry: 
        ...
        result_printout =( ['Entered Start Date: ' + start,
            'The lowest Temperature was: ' + str(timin) + " F",
...
@approute("/api/v1.-/<start>/<end>") #Calculate the 'TMIN', 'TAVG', AND 'TMAX' for dates between the start and end date inclusive
...
def start_end(start, end):
    ...
    # Date Range (only for help to user in case date gets entered wrong)
    date_range_max = session.query(Measurement.date).order_by(Measurement.date.desc()).first()
    date_range_min_str = str(date_range_max)
    date_range_max_str = re.sub("'|,", "",date_range_max_str)
    print(date_range_max_str)
